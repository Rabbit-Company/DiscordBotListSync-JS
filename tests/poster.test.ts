...
import { BotListError, DiscordBotListCom, DiscordExtremeListXyz, DiscordsCom, StatsPoster, TopGG } from "../src/index";
...
function mockFetch(handler: (req: CapturedRequest) => Response | Promise<Response>): CapturedRequest[] {
...
describe("TopGG adapter", () => {
	...
	it("throws BotListError with status and body on failure", async () => {
		...
		const err = await new TopGG({ token: "t" }).post({ guildCount: 1 }).catch((e) => e);
...
describe("DiscordBotListCom adapter", () => {
	it("sends a POST with bearer auth, the bot id and supported stats", async () => {
		const calls = mockFetch(() => new Response('{"success":true}', { status: 200 }));
		const list = new DiscordBotListCom({ token: "dbl-token" });
		...
		expect(JSON.parse(call.body!)).toEqual({
			guilds: 321,
			users: 12_345,
			voice_connections: 17,
		});
	});

	it("omits unsupported and unavailable optional stats", async () => {
		const calls = mockFetch(() => new Response('{"success":true}', { status: 200 }));

		await new DiscordBotListCom({ token: "t" }).post({ guildCount: 5, shardCount: 2 }, { botId: "42" });

		expect(JSON.parse(calls[0]!.body!)).toEqual({ guilds: 5 });
	});

	it("refuses to post without a botId", async () => {
		mockFetch(() => new Response(null, { status: 200 }));
		const err = await new DiscordBotListCom({ token: "t" }).post({ guildCount: 1 }).catch((e) => e);
		expect(err).toBeInstanceOf(TypeError);
		expect(String(err)).toContain("client ID");
	});
});

describe("DiscordExtremeListXyz adapter", () => {
	it("sends a POST with bearer auth, the bot id and shard count", async () => {
		const calls = mockFetch(() => new Response('{"error":false,"status":200}', { status: 200 }));
		const list = new DiscordExtremeListXyz({ token: "del-token" });

		await list.post({ guildCount: 456, shardCount: 8, userCount: 9999 }, { botId: "111222333" });
	...
	it("omits shardCount when not provided", async () => {
		const calls = mockFetch(() => new Response('{"error":false,"status":200}', { status: 200 }));

		await new DiscordExtremeListXyz({ token: "t" }).post({ guildCount: 5 }, { botId: "42" });

		expect(JSON.parse(calls[0]!.body!)).toEqual({ guildCount: 5 });
	});

	it("refuses to post without a botId", async () => {
		mockFetch(() => new Response(null, { status: 200 }));
		const err = await new DiscordExtremeListXyz({ token: "t" }).post({ guildCount: 1 }).catch((e) => e);
		expect(err).toBeInstanceOf(TypeError);
		expect(String(err)).toContain("client ID");
	});
});
...
describe("DiscordsCom adapter", () => {
	it("sends a POST with raw token auth and the bot id in the URL", async () => {
		const calls = mockFetch(() => new Response('{"status":"success"}', { status: 200 }));
		const list = new DiscordsCom({ token: "discords-token" });

		await list.post({ guildCount: 123, shardCount: 4 }, { botId: "111222333" });

		const call = calls[0]!;
		expect(call.url).toBe("https://discords.com/bots/api/bot/111222333/setservers");
		expect(call.method).toBe("POST");
	...
	it("refuses to post without a botId", async () => {
		mockFetch(() => new Response(null, { status: 200 }));
		const err = await new DiscordsCom({ token: "t" }).post({ guildCount: 1 }).catch((e) => e);
		expect(err).toBeInstanceOf(TypeError);
		expect(String(err)).toContain("client ID");
	});
});
...
describe("StatsPoster", () => {
	...
	it("postNow posts to all lists and reports per-list results", async () => {
		const calls = mockFetch((req) =>
			req.url.includes("top.gg") ? new Response(null, { status: 204 }) : new Response('{"message":"missing_authorization"}', { status: 401 }),
		);
